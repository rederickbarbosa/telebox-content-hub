-- Primeiro verificar os dados existentes que podem estar causando problema
-- e corrigir antes de aplicar a constraint

-- 1. Verificar dados existentes na tabela apps
SELECT tipo, COUNT(*) as quantidade FROM apps GROUP BY tipo;

-- 2. Corrigir os dados existentes que não estão em conformidade
UPDATE apps SET tipo = 'premium' WHERE tipo NOT IN ('gratuito', 'premium');

-- 3. Agora aplicar a constraint correta
...
-- 4. Inserir configurações iniciais do admin
